...
//   credentials: fromIni({ profile: process.env.AWS_PROFILE || '789279989416_AdministratorAccess' }), // 明示的にプロファイル指定
//   // Claude は us-east-1 で利用可能
...
export const bedrockClient = new BedrockRuntimeClient({
  region: 'us-east-1',
  credentials: fromIni({
    profile: process.env.AWS_PROFILE || '789279989416_AdministratorAccess',
  }),
});
...
// Claude のレスポンステキストから ```json や ``` を除去
function cleanClaudeResponse(text: string): string {
  return text
    .replace(/^```(?:json)?\s*/i, '') // 先頭の ```json または ``` を除去
    .replace(/```$/, '')              // 最後の ``` を除去
...
export async function invokeClaude(prompt: string): Promise<string> {
  ...
  for (const modelId of CLAUDE_MODELS) {
    try {
      ...
      const cleanedText = cleanClaudeResponse(rawText); // ← 修正ポイント
      console.log(`Successfully used model: ${modelId}`);
      return cleanedText;
...
async function invokeClaudeWithModels(prompt: string, modelIds: string[]): Promise<string> {
  ...
  for (const modelId of modelIds) {
    try {
      ...
      const rawText = responseBody.content[0].text;
      const cleanedText = cleanClaudeResponse(rawText); // ← 同様にクリーンアップ
      console.log(`Successfully used model: ${modelId}`);
      return cleanedText;
